	...
	-- nvim-cmp provides auto completion and auto completion dropdown ui
	{
		...
		config = function()
			...
			cmp.setup({
				...
				snippet = {
					expand = function(args)
						require("luasnip").lsp_expand(args.body) -- Si usas luasnip para snippets
					end,
				},
				mapping = cmp.mapping.preset.insert({
					["<Tab>"] = cmp.mapping.select_next_item({ behavior = cmp.SelectBehavior.Select }),
					["<S-Tab>"] = cmp.mapping.select_prev_item({ behavior = cmp.SelectBehavior.Select }),
					["<CR>"] = cmp.mapping.confirm({ select = true }), -- Esto confirma la selección predefinida
					["<C-Space>"] = cmp.mapping.complete(),
					["<Esc>"] = cmp.mapping.abort(), -- Para salir del menú de autocompletado
				}),
				sources = cmp.config.sources({
					{ name = "nvim_lsp" }, -- Integración con LSP
					{ name = "luasnip" }, -- Snippets
				}, {
					{ name = "buffer" }, -- Palabras del buffer
					{ name = "path" }, -- Archivos y rutas
				}),
				preselect = cmp.PreselectMode.Item, -- Preseleccionar la primera opción
				formatting = {
					format = function(entry, vim_item)
						-- Personaliza el formato de las sugerencias aquí si lo deseas
						return vim_item
					end,
				},
				experimental = {
					ghost_text = true, -- Muestra texto fantasma para completar
				},
			})
		end,
	},
